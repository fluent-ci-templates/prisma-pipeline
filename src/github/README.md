...
The following command will generate a `.github/workflows/prisma-migrate.yml` file in your project:

```bash
fluentci gh init -t prisma_pipeline
```

Or, if you already have a `.fluentci` folder (generated from `fluentci init -t prisma`) in your project:
